...
# Sentence Generator (Decoder) for GPT-3 ...
def decoder_for_gpt35_chatanywhere(args, demo, input, max_length):
    ...
    if args.model == "gpt-3.5-turbo":
        ...
        raise ValueError("model is not properly defined by gpt-3.5-turbo ...")
    ...
    if ("few_shot" in args.method or "auto" in args.method)  and engine == "code-davinci-002":
        response = client.chat.completions.create(
          model=engine,
          messages=[{'role': 'system', 'content': demo}, {'role': 'user', 'content': input}],
          max_tokens=max_length,
          temperature=args.temperature,
          top_p=1,
          frequency_penalty=0,
...
class Decoder():
    ...
    def decode(self, args, sys_p, input, max_length):
        response = decoder_for_gpt35_chatanywhere(args, sys_p, input, max_length)
        return response
